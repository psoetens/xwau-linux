...
```bash
curl -fL https://github.com/psoetens/xwau-linux/archive/refs/tags/v0.5.1.tar.gz | tar xz
cd xwau-linux-0.5.1
```

That tag's scripts are pinned to download the matching prebuilt binaries, so the
two always stay in sync. For a different version, swap `v0.5.1` for any tag on
...
```bash
cd xwau-linux-0.5.1                        # the newer release you downloaded
